...
import { influencerPagesData } from './influencerPagesData';
import { 
  ChevronDown, ArrowRight, Check, Users, 
  Share2, MessageCircle, BarChart3, Video, 
  Globe, ShieldCheck, Zap, Flame, Compass
} from 'lucide-react';
...
const ICON_POOL = [
  Users, Share2, MessageCircle, BarChart3, Video, 
  Globe, ShieldCheck, Zap, Flame, Compass
];

interface InfluencerServicePageProps {
  type: string;
}

export function InfluencerServicePage({ type }: InfluencerServicePageProps) {
  const { setContactModalOpen } = useAppStore();
  const [activeFaq, setActiveFaq] = useState<number | null>(null);
  const containerRef = useRef<HTMLDivElement>(null);

  const data = influencerPagesData[type];
  ...
  useEffect(() => {
    if (data) {
      ...
      if (metaDesc) {
        metaDesc.setAttribute('content', data.metaDesc || "Eon8 influencer marketing and promotion services");
      }
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // Hero Title entrance
      gsap.fromTo('.inf-hero-content',
        { opacity: 0, y: 50 },
        { opacity: 1, y: 0, duration: 1.0, ease: 'power3.out' }
      );

      // Staggered cards reveal
      gsap.fromTo('.inf-card-reveal',
        { opacity: 0, y: 40 },
        {
          opacity: 1,
          y: 0,
  ...
  if (!data) {
    return (
      <div className="py-32 text-center">
        ...
        <p className="text-muted-foreground mt-2">The requested influencer service page could not be located.</p>
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="bg-background text-foreground overflow-hidden font-body">
      ...
      <section className="relative py-28 md:py-36 bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 text-white border-b border-blue-500/10">
        ...
        <div className="max-w-5xl mx-auto px-6 text-center space-y-6 relative z-10 inf-hero-content">
      ...
      <section id="services" className="py-24 bg-slate-50 dark:bg-slate-900/40 border-y border-border/30 px-6 inf-cards-trigger">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.services.map((srv, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className="inf-card-reveal bg-white dark:bg-slate-950 border border-border/40 hover:border-orange-500/30 p-8 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between text-left"
                >
      ...
      {data.processSteps && data.processSteps.length > 0 && (
        <section className="py-24 px-6 max-w-6xl mx-auto space-y-16 inf-steps-trigger">
          <div className="text-center space-y-3">
            <p className="text-orange-500 font-bold uppercase tracking-wider text-xs">Execution Roadmap</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.processSteps.map((st, idx) => (
              <div 
                key={idx}
                className="inf-step-reveal bg-card/40 border border-border/40 hover:border-blue-500/20 p-8 rounded-3xl relative text-left shadow-sm hover:shadow-md transition-shadow"
              >
      ...
      <section className="py-24 bg-slate-950 dark:bg-slate-900 text-white px-6 relative">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-8 relative z-10">
          ...
          <p className="text-slate-400 text-sm md:text-base leading-relaxed max-w-xl mx-auto text-center">
            Partner with leading social media creators and run high-performance campaign strategies that put your brand in front of millions.
          </p>
